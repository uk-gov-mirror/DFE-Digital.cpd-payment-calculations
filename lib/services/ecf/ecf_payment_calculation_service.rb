...
private
  ...
  def service_fee_monthly
    ...
    (service_fee_total / number_of_monthly_payments).round(0).to_d
  end
  ...
  def variable_payment_schedule
    @config.dig(:retained_participants).each_with_object({}) do |(payment_type, retained_participants), result|
      result[payment_type] = {
        ...
        per_participant: variable_payment_schedule_per_participant(payment_type),
        subtotal: variable_payment_schedule_subtotal(payment_type, retained_participants),
      }
    end
  end

  def variable_payment_schedule_per_participant(payment_type)
    variable_payment_per_participant * (payment_type.match(/Start|Completion/) ? 0.2 : 0.15)
  end

  def variable_payment_schedule_subtotal(payment_type, retained_participants)
    variable_payment_schedule_per_participant(payment_type) * retained_participants
  end
end
